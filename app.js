// app.js - Premium-Edition (Optimiert: Ohne Admin-Filter, mit verkürztem & rechtsbündigem Progress-Balken)

window._initApp = async function(u) {
    _injectPremiumStyles(); // Injiziert alle flüssigen Animationen und modernen UI-Komponenten
    ...
    _updateDatalists(); // Initialisiert die "Zuletzt verwendet"-Historie
    _loadUserEntries(u);
...
// --- HISTORIE-FUNKTION (ZULETZT VERWENDET) ---
...
async function _saveEntry() {
    ...
    if (dayOfWeek !== 0 && dayOfWeek !== 3 && dayOfWeek !== 5 && dayOfWeek !== 4) {
        return _toast("Fehler: Schießnachweis nur für Mittwoch, Freitag oder Sonntag zulässig!", "error");
    }

    if (dayOfWeek === 4 && !f.b.value.trim()) {
        return _toast("Für Einträge am Donnerstag MUSS eine Bemerkung eingetragen werden!", "error");
    }
    ...
    const { error } = await window._c.from("entries").insert([{ 
        user_id: user.id, datum: f.d.value, gastschuetze: f.g.value.trim(), 
        schiessstand: f.s.value, disziplin: f.di.value.trim(), waffe: f.w.value.trim(), 
        aufsicht: f.au.value.trim(), bemerkung: f.b.value.trim(), status: 'offen' 
    }]);
...
async function _loadUserEntries(u) {
    if (!u) { const { data: { user } } = await window._c.auth.getUser(); u = user; }
    ...
    for (let m = 0; m <= currentMonth; m++) {
        if (!monthsWithEntries.has(m)) { istRegelmaessig = false; break; }
    }
    ...
    if (dbCircle) {
        ...
        dbCircle.style.color = color === "var(--y)" ? "#000" : "#fff";

        // --- DYNAMISCHER MODERN PROGRESS BAR (KÜRZER & RECHTSBÜNDIG) ---
        let pWrap = document.getElementById("_db_p_wrap");
        if (!pWrap) {
            pWrap = document.createElement("div"); pWrap.id = "_db_p_wrap"; pWrap.className = "_progress_container";
        ...
        setTimeout(() => { pBar.style.width = pct + "%"; pBar.style.backgroundColor = color; }, 100);
    }
    ...
    if (data) {
        data.forEach(e => {
            ...
            if (e.datum && !e.datum.startsWith(currentYear.toString())) {
                i.classList.add("_older_entry"); i.style.display = "none";
                hasOlderEntries = true; olderEntriesCount++;
            }
            l.appendChild(i);
        });
...
async function _refreshOpenRequests() {
    ...
    if (adminTab) {
        if (!badge) {
            badge = document.createElement("span"); badge.id = "_admin_badge";
            badge.style.cssText = "background-color: var(--r); color: white; border-radius: 10px; padding: 2px 6px; font-size: 0.75rem; margin-left: 6px; font-weight: bold; display: inline-block;";
    ...
    if (e && e.length > 0) {
        e.forEach(x => {
            ...
            // E-Mail ist jetzt im gefärbten Kasten eingebettet mit einer optischen Trennlinie
            i.innerHTML = `
                <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 10px;">
                    <span style="color: var(--t); font-weight: bold; font-size: 0.95em;">${formattedDate}</span>
                ...
                <div class="_card_details" style="background: ${isRegular ? '#f0f0f0' : '#faeedf'}; padding: 12px; border-radius: 6px;">
                    <strong style="color: var(--t); word-break: break-all; font-size: 0.95em; display: block; margin-bottom: 8px; border-bottom: 1px solid ${isRegular ? '#e0e0e0' : '#eedcc5'}; padding-bottom: 6px;">
                        ${r ? r.email : 'Unbekannt'}
                    </strong>
...
function _triggerConfetti() {
    for (let i = 0; i < 40; i++) {
        const p = document.createElement("div"); p.className = "_confetti";
...
async function _getExportData() {
    ...
    return entries.map(row => {
        ...
        return {
            Datum: row.datum, Email: u ? u.email : 'Unbekannt', Schießstand: row.schiessstand,
            Disziplin: row.disziplin, Waffe: row.waffe, Aufsicht: row.aufsicht, Gast: row.gastschuetze || '-', Status: row.status
        };
    });
}

document.getElementById("_xe").onclick = async () => {
    const data = await _getExportData(); if (!data) return;
...
document.getElementById("_pdf").onclick = async () => {
    const data = await _getExportData(); if (!data) return;
    const select = document.getElementById("_ms"); const email = select.options[select.selectedIndex].text;
...
function _startInactivityTimer() {
    let t; const r = () => { clearTimeout(t); t = setTimeout(() => { window._c.auth.signOut(); location.reload(); }, 300000); };
...
function _injectPremiumStyles() {
    ...
    const s = document.createElement("style"); s.id = "_premium_styles";
    s.innerHTML = `
        /* Toast Notifications */
        ...
        /* Gekürzter & nach rechts verschobener Progress Bar für ein entzerrtes UI */
        ...
        /* Dashboard Anzahl-Badge ebenfalls nach rechts rücken */
        #_db_circle { margin-left: auto !important; display: flex; align-items: center; justify-content: center; transition: all 0.3s ease; }
        ...
        ._btn_active { background-color: var(--t, #34495e) !important; }
